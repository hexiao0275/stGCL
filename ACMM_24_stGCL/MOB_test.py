...
if __name__ == "__main__":
    ...
    for i in range(len(datasets)):
        dataset = datasets[i]
        savepath = 'Spatial-MGCN/result/' + dataset + '/'
        ...
        adata.obsm['emb'] = emb
        pd.DataFrame(adata.obsm['emb']).to_csv(savepath + 'Spatial_MGCN_emb.csv', header=None, index=None)


        adata = mclust_R(adata, num_cluster=config.class_num)
        pd.DataFrame(adata.obs['mclust']).to_csv(savepath + 'Spatial_MGCN_idx.csv', header=None, index=None)
