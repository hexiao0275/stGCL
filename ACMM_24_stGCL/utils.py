...
def dicr_loss(Z_ae, Z_igae):
    loss_dicr = F.mse_loss(Z_ae, Z_igae)
    return loss_dicr

def TailClusterLoss(features):
    ...
    # 计算每个样本到聚类中心的欧氏距离
    euclidean_distance = torch.norm(features - cluster_center, dim=1)
    # 计算分布
    distribution = F.softmax(features, dim=1)
    # 计算聚类损失
    cluster_loss = torch.mean(euclidean_distance)
    # 计算分布损失，使用负熵作为衡量稀疏性的指标
    distribution_loss = -torch.mean(torch.sum(distribution * torch.log(distribution + 1e-8), dim=1))
    # 最终损失
    total_loss = alpha * cluster_loss + beta * distribution_loss
